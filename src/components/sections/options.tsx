...
import { getComplexityInfo, formatCurrency } from "@/utils";
...
type OptionsSectionProps = {
  ...
  expandedOptions: Record<number, boolean>;
                ...
                <Fragment key={index}>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {opt.disableMaxMultiplier
                        ? `${opt.hoursMin}h`
                        : `${opt.hoursMin}h - ${opt.hoursMax}h`}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {opt.disableMaxMultiplier
                        ? `$${formatCurrency(opt.costMin)}`
                        : `$${formatCurrency(opt.costMin)} - $${formatCurrency(opt.costMax)}`}
                    </td>
                            ...
                            <table className="min-w-full">
                              <thead>
                                <tr className="text-xs uppercase text-gray-500">
                                  <th className="px-4 py-2 text-left">Vue/Écran</th>
                              ...
                              <tbody>
                                {opt.subSteps.map((s, i) => (
                                  <tr key={i} className="border-t text-sm">
                                    <td className="px-4 py-2 font-medium text-gray-900">
                                      {s.title}
                                    </td>
                                    <td className="px-4 py-2 text-gray-600">{s.hours}h</td>
                                    <td className="px-4 py-2">
                                      <span
                                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
                                          s.complexity === "Élevée"
                                            ? "bg-red-100 text-red-800"
                                            : s.complexity === "Moyenne"

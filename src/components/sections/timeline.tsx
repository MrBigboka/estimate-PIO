...
import {
  PROJECT_SCHEDULE_VARIANTS,
  TIMELINE_DATA_VARIANTS,
} from "@/config";
import { formatCurrency } from "@/utils";

type StepForAllocation = { id: number; name: string; color: string; hoursMin: number };
  ...
  return (
    <div className="space-y-8 transition-all duration-300 ease-out animate-in fade-in slide-in-from-bottom-2">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <ChartContainer config={monthlyChartConfig} className="h-80 w-full">
          <BarChart data={monthlyCostData}>
            ...
            <Bar dataKey="min" name={hasRange ? "Min" : "Coût"} fill={hasRange ? "#93C5FD" : "#2563EB"} radius={4} />
            {hasRange && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white rounded-lg shadow-lg p-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Calendrier du projet</h2>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <ChartContainer config={cumulativeChartConfig} className="h-[400px] w-full">
